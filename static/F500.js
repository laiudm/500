...
// when dragging & opening a gap, close the gap when another drag-target is hit
//
// Improve kitty and play-areas graphically
// View-all-cards
...
class PlayArea extends Component {
	...
	render(props, state) {
		return h('div', {className: "playArea"},
...
class FiveHundred extends Component {
	...
	onMouseMove(e) {	// called whenever the mouse moves, whether mouse button is clicked or not
		...
		if (this.state.lastID != id) {	// log change only, otherwise it happens on each movement
			this.lastID = id;
			if (id != null) {
		...
		if (e.buttons == 1 && this.state.dragSource.startsWith('drag-P') && id.startsWith('drag-P') ) {
			// I'm dragging my card over one of my cards with the mouse button down
	...
	render(props, state) {
		...
		return h('div', { className: "C500", 
			...
			h(DraggedCard, {x: this.state.mouse.x-20, y: this.state.mouse.y-20, card: this.state.dragCard, visible: 'true'}),
		);
	}		
}
